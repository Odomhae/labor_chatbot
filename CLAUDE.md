...
Anthropic Claude API(`claude-haiku-4-5-20251001`)로 PDF 내용을 기반으로 자연어 응답을 제공한다. Vercel에 배포한다.
...
- `server.js`: 유일한 서버 진입점. PDF 로딩·파싱과 Anthropic API 호출을 모두 담당한다.
- `public/`: 정적 파일(HTML/CSS/JS). 빌드 단계 없이 Express가 직접 서빙한다.
- `docs/`: PDF 원본 파일 보관. 서버 시작 시 파싱하여 컨텍스트로 사용한다.
- Anthropic API 키는 `server.js`에서만 참조(`process.env.ANTHROPIC_API_KEY`). 프론트엔드에 절대 노출하지 않는다.
